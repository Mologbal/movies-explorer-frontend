export const MAIN_API_OPTIONS = {
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
    },
  };
  export const MOVIES_API_OPTIONS = {
    method: 'GET',
  };
  export const API_MAIN_URL = 'http://localhost:3002';
  export const API_MOVIES_URL = 'https://api.nomoreparties.co/beatfilm-movies';
  export const IMAGES_URL = 'https://api.nomoreparties.co';
  export const MAX_SHORT_MOVIE_LENGTH = 40;
  export const DESKTOP_NUM_OF_ADDED_MOVIES = 3;
  export const TABLET_NUM_OF_ADDED_MOVIES = 2;
  export const MOBILE_NUM_OF_ADDED_MOVIES = 1;